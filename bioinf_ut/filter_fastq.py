from bioinf_ut.modules_4main_script.module_for_filter_fastq import length_bounds_function
...
def filter_fastq(fastq_file, gc_bounds=(0, 100), length_bounds=(0, 2 ** 32), quality_threshold=0):
    ...
    for seq_name, seq_1 in fastq_file.items():
        ...
        if (gc_bounds_l <= result_bounds <= gc_bounds_u and
                gc_bounds_l <= length_seq <= length_bounds_u and
                result_quality >= quality_threshold):
            result_filtering[seq_name] = (seq, quality)
    return result_filtering

from bioinf_ut.modules_4main_script.module_for_dna_rna_tools import transcribe, reverse, complement, reverse_complement
